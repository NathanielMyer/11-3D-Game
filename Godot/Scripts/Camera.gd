...
export var center_offset : Vector2 = Vector2(0, 2)
export(float, 0.1, 1.0) var move_speed : float = 0.62
export(float, 0.1, 0.4) var smoothing : float = 0.03
...
func calculate_center(rect: Rect2) -> Vector2:
	return Vector2(rect.position.x + rect.size.x / 2, rect.position.y / 2)
